...
tags: Cluster
mathjax: true
figure: /images/spectral-cluster/spectral_clustering_3.png
---
* content
{:toc}

论文题目： [A tutorial on spectral clustering](https://dl.acm.org/citation.cfm?id=1288832)
...
用$G=(V,E)$来表示一个无向图，它的顶点集合为$V=\{v_1,...,v_n\}$.我们假设图$G$是赋权的，即对任意两个顶点$v_i,v_j$都存在一个非负权重$w_{ij}\geq0$.图的加权邻接矩阵可以表示为$W=(w_{ij})_{i,j=1,...,n}$. 

$w_{ij}=0$意味着顶点$v_i,v_j$之间没有任何边连接.因为$G$是无向图，因此$W$是对称阵.

我们定义$v_i\in V$的度为$d_i=\sum_{j=1}^nw_{ij}$,同时定义度矩阵$D=diag(d_1,...,d_n)$.给定顶点子集$A\subset V$，我们定义它的补集$\bar{A}=V\backslash A$.

我们定义A的指标向量为$1_{A}=(f_1,...,f_n)'\in R^n,f_i=1,iff\ v_i\in A,or\ f_i=0$.同时我们记$i\in A=\{i \vert v_i\in A\}$.此时对于任意两个集合(这两个集合并非是不相交的)，我们可以定义

$$
W(A,B)=\sum_{i\in A,j\in B}w_{ij}
$$

同时我们给出两个可以度量子集$A\subset V$大小的方法：

$\ \vert  A\ \vert =$A中顶点的个数

$vol(A)=\sum_{i\in A}d_i$

直观上$\ \vert A\ \vert $通过$A$中顶点的个数度量了A的大小，同时$vol(A)$通过计算A中顶点的度对$A$的大小进行了度量.

子集$A\subset V$称为连通图，如果A中任意两个顶点都可以被一个路径所连接，同时路径里的所有点都在$A$里.

子集$A\subset V$被称为连通分支（这里应该理解为$A,\bar{A}$是$V$的连通二部图），如果它是连通的，而且$A,\bar{A}$之间没有连通路径.非空集合$A_1,...,A_k$形成了一个图的割，如果它们满足$A_i\cap A_j=\emptyset$，且$A_1\cup...\cup A_k=V$.
...
* 全连接图
  ...
  这里我们直接用正相似度把所有的点彼此连接，我们用$s_{i,j}$来给所有的边赋权.注意到我们上面强调的图应该表现局部的相邻关系，因此这里的相似度也要表现这种关系.我们一般用的相似度函数是高斯相似度函数$s(x_i,x_j)=exp(\frac{-\ \vert x_i-x_j\ \vert ^2}{2\sigma^2})$,这里我们引入了超参数$\sigma$来控制相邻关系的幅度(后面会讲到增大超参数$\sigma$在某种程度上可以视作增大$\epsilon-neighborhood$图中的$\epsilon$)
...
如上文定义，$D=diag(d_1,...,d_n)$.我们给出下列与谱聚类理论分析有关的命题并证明.
...
* 命题1
  ...
  1. 对于任意向量$f\in R^n$，我们有$f'Lf=\frac{1}{2}\sum_{i,j=1}^nw_{ij}(f_i-f_j)^2$
      ...
      $f'Lf=f'Df-f'Wf=\sum_{i=1}^nd_if_i^2-\sum_{i,j=1}^nf_iw_{i,j}f_j= \\
      \frac{1}{2}(\sum_{i=1}^nf_i^2d_i-2\sum_{i,j}^nf_if_jw_{ij}+\sum_{j=1}^nf_j^2d_j)$
  ...
  3. $L$的最小特征值是0，存在$1_{V}$的特征向量.

      显然

  4. $L$有非负实值的特征值$0=\lambda_1\leq \lambda_2\leq \lambda_n$

我们利用这些特性来展开谱聚类分析,并用以上命题得到这样一个非常重要的结论：

* 命题2

  假设G是一个无向图，边权矩阵W为正.那么特征根0的幂次k等于这个图里面的连通分支$A_1,..,A_k$的个数.同时特征值0的特征空间由连通分支的指标向量$\mathcal{1}_{A_1},\mathcal{1}_{A_2},...,\mathcal{1}_{A_k}$所扩张而成.
  ...
  **证明：**
  ...
  因此在$w_{ij}>0$的地方$f_i=f_j$.如果$k=1$的话，说明0的特征向量只有一个$1_{V}$，那么$w_{ij}>0,\forall i,j$，因此这个图有且只有1个连通分支.
  
  反之如果有1个连通分支的话，那就意味着$w_{ij}>0$，也就是$k=1$.充要性得证.
...
* 命题3：
  ...
  4. 0是 $L_{rw}$ 的特征值，同时 $\mathcal{1}_{V}$ 为 $L_{rw}$ 的特征向量.0为 $L_{sym}$ 的特征值，同时 $D^{1/2}\mathcal{1}_{v}$ 为 $L_{sym}$ 的特征向量.
...
* 命题4
  ...
  令G为有非负权值的无向图，那么特征根中0的幂次k等价于图中的连通分支$A_1,...,A_k$的数目.对于正则拉普拉斯图矩阵$L_{rw}$而言，0的特征空间由$\mathcal{1}_{A_{i}}$的这样一组指标向量扩展而成.同时对于$L_{sym}$而言，0的特征空间由 $D^{1/2}\mathcal{1}_{A_{i}}$ 而扩展而成.
...
输入:相似度矩阵$S\in R^{n*n}$以及要分为的类数*k*
...
7. 输出$A_1,...,A_k$满足$A_i=\{j \vert y_j\in C_i\}$
...
在我们进入谱聚类的理论解释部分之前，我们想用一个非常简单的例子来展示谱聚类的作用.这个例子将在文章中多次使用，因为它很好地绘出了类别之间的数量关系.这个数据集是一个含有200个数据点的数据集，它们由4个高斯分布混合而成.在处理这个数据集的过程中，我们选择相似度函数为$s(x_i,x_j)=exp(-\ \vert x_i-x_j\ \vert ^2/(2\sigma ^2)),\sigma=1$.
...
![1524449771309](/images/spectral-cluster/spectral_clustering_1.png)
...
-\sqrt{\ \vert A\ \vert /\ \vert \bar{A}\ \vert }&
\text{if $v_i\in \bar{A}$}.
\end{cases}
$$
...
=cut(A,\bar{A})(\frac{\ \vert \bar{A}\ \vert }{\ \vert A\ \vert }+\frac{\ \vert A\ \vert }{\ \vert \bar{A}\ \vert }+2)\\
= \vert V \vert RatioCut(A,\bar{A})
$$
...
此时$D^{-1/2}LD^{-1/2}=L_{sym}$,而同时

$$
D^{1/2}\ 1_{V}
$$

是$L_{sym}$的0特征值的特征根，因此根据我们在*RatioCut*中所提到的定理，我们可以得出它的松弛解为$L_{sym}$的第2个特征向量.而利用$L_{rw},L_{sym}$之间的关系我们可以得出此时$f$为$L_{rw}$的第2个最小特征值所对应的特征向量.
...
![1524486923878](/images/spectral-cluster/spectral_clustering_2.png)
...
如果我们的数据点都可以表示为欧式空间$R^d$中的点，那么一个比较具有可解释性的相似函数是$s(x_i,x_j)=exp(-\ \vert x_i-x_j\ \vert )^2/(2\sigma^2)$,当然我们仍然要对$\sigma$进行选取.
...
这里我们没有给全连接图的例子.一般全连接图就是用高斯相似函数$s(x_i,x_j)=exp(-\ \vert x_i-x_j\ \vert )^2/(2\sigma^2)$所诱导出的全连接图.这里参数$\sigma$扮演了与$\epsilon-$邻接图一样的角色.局部相邻区域的点之间用比较高的权重来进行互相连接，而相隔很远的点权重非常接近于0.但是它所得到的相似矩阵并不是一个稀疏矩阵.
...
在实现谱聚类的过程中，我们需要计算一个很大的Laplace矩阵的前k个特征向量.如果我们用$k-$近邻图或者是$\epsilon$邻接图，那么L矩阵是一个稀疏矩阵.我们已经有了一些有效的方法来计算特征根，而这些算法的收敛速率取决于特征差 

$$
\gamma_k=\ \vert \lambda_k-\lambda_{k+1}\ \vert 
$$

更大的特征差意味着前k个特征值更快收敛.

但是当我们遇到一个特征根要比前一个大很多倍的时候可能会导致一些问题.比如在k个连通分支的理想情况下，特征根0有k次幂，此时特征空间由k个特征向量所张成.但是在实际计算的过程中，这些数值解并不一定收敛到这些特征向量，相反，这些特征向量可能会收敛到一组特征空间的正交基.但是它并不会影响什么.注意到由
...
$$
u=\sum_{i=1}^{k}a_{i}\ 1_{A_i}$$

因此在聚类方面就算它是其它向量的线性组合也没有关系.因此所返回的特征向量仍然编码了聚类信息，所以它们可以被*k-means*来重构类别.
...
![1524557780076](/images/spectral-cluster/spectral_clustering_4.png)
...
![1524570337393](/images/spectral-cluster/spectral_clustering_5.png)
...
对比$L_{rw},L_{sym}$的不同之处以及谱聚类算法，3个对谱聚类的数学解释都倾向于采用$L_{rw}$，原因是$L_{rw}$的特征向量就是聚类的指标向量$\mathcal{1}_{A_{i}}$，而$L_{sym}$的特征向量前要乘$D^{1/2}$，这可能会带来一些意外.因为使用$L_{sym}$并不会减少计算负担，因此我们推荐使用$L_{rw}$.
