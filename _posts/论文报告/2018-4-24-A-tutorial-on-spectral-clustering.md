...
* 命题2
  ...
  **证明：**
  ...
  假设$k=1$，同时注意到$0=f'Lf=\sum_{i,j=1}^nw_{ij}(f_i-f_j)^2$，因此在$w_{ij}>0$的地方$f_i=f_j$.如果k=1的话，说明0的特征向量只有一个 $1_{V}$ ，那么$w_{ij}>0,\forall i,j$，因此这个图有且只有1个连通分支.反之如果有1个连通分支的话，那就意味着$w_{ij}>0$，也就是k=1.充要性得证.
